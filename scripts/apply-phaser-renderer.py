...
root=Path('.')
old=(root/'src/app/match-2d-pitch.tsx').read_text()
(root/'src/app/match-2d-legacy.tsx').write_text(old.replace('export default function Match2DPitch','export default function LegacyMatch2DPitch'))
...
export default function PhaserMatchRenderer({session,home,away,onFailure}:{session:LiveMatchState;home:LeagueClub;away:LeagueClub;onFailure:()=>void}){
 ...
 useEffect(()=>{let disposed=false;if(!host.current)return;void import("phaser").then(Phaser=>{if(disposed||!host.current)return;const parent=host.current;class MatchScene extends Phaser.Scene{
   playerObjects=new Map<string,{dot:Phaser.GameObjects.Arc;label:Phaser.GameObjects.Text}>();ball?:Phaser.GameObjects.Arc;trail?:Phaser.GameObjects.Graphics;actionText?:Phaser.GameObjects.Text;lastSize="";
   ...
   update(time:number){const snap=bridge.current.snapshot,w=this.scale.width,h=this.scale.height,size=`${w}x${h}`;if(size!==this.lastSize){this.lastSize=size;this.children.removeAll(true);this.playerObjects.clear();this.drawPitch();this.ball=this.add.circle(0,0,5,0xffffff).setStrokeStyle(1,0x111111,1).setDepth(20);this.trail=this.add.graphics().setDepth(3);this.actionText=this.add.text(14,12,"",{fontFamily:"Arial",fontSize:"12px",fontStyle:"bold",color:"#ffffff",backgroundColor:"rgba(2,10,7,.62)",padding:{x:7,y:4}}).setDepth(30);this.syncPlayers()}this.syncPlayers();const elapsed=Math.max(0,(time-snap.sceneStart)/1000),current=sceneActionAt(snap.plan,elapsed),p=scenePoint(current.action,current.progress),defSide:MatchSide=snap.plan.attackingSide==="home"?"away":"home",pressers=new Set(snap.actors.filter(a=>a.side===defSide).sort((a,b)=>Math.hypot(a.base.x-p.x,a.base.y-p.y)-Math.hypot(b.base.x-p.x,b.base.y-p.y)).slice(0,2).map(a=>a.id));
    for(const a of snap.actors){const o=this.playerObjects.get(a.id);if(!o)continue;let{x,y}=a.base,hasBall=a.side===snap.plan.attackingSide,dir=a.side==="home"?-1:1;if(hasBall){y=clamp(y+dir*3);x=clamp(mix(x,p.x,.04))}else{x=clamp(mix(x,p.x,pressers.has(a.id)?.24:.07));y=clamp(mix(y,p.y,pressers.has(a.id)?.17:.05))}if(a.id===current.action.actorId){x=mix(current.action.from.x,current.action.to.x,Math.min(.84,current.progress*.75));y=mix(current.action.from.y,current.action.to.y,Math.min(.84,current.progress*.75))}if(a.id===current.action.receiverId){x=mix(a.base.x,current.action.to.x,.78);y=mix(a.base.y,current.action.to.y,.78)}if(a.id===current.action.defenderId){const r=["save","block","interception","tackle"].includes(current.action.kind)?.9:.5;x=mix(a.base.x,current.action.to.x,r*current.progress);y=mix(a.base.y,current.action.to.y,r*current.progress)}const X=px(x,w),Y=px(y,h);o.dot.setPosition(X,Y);o.label.setPosition(X,Y+14);const active=a.id===current.action.actorId,receiver=a.id===current.action.receiverId,defender=a.id===current.action.defenderId;o.dot.setScale(active?1.28:receiver||defender?1.14:1);o.dot.setAlpha(1)}
    this.ball?.setPosition(px(p.x,w),px(p.y,h));this.trail?.clear();this.trail?.lineStyle(2,0xffffff,.16);this.trail?.lineBetween(px(current.action.from.x,w),px(current.action.from.y,h),px(current.action.to.x,w),px(current.action.to.y,h));this.actionText?.setText(`${snap.minute}′  ${current.index+1}/${snap.plan.actions.length}  ${current.action.label}`)}
  }
...
'''
(root/'src/app/match-2d-phaser.tsx').write_text(phaser)
...
export default function Match2DPitch(props:{session:LiveMatchState;home:LeagueClub;away:LeagueClub}){
 const [fallback,setFallback]=useState(false),fail=useCallback(()=>setFallback(true),[]);
 return fallback?<LegacyMatch2DPitch {...props}/>:<PhaserMatchRenderer {...props} onFailure={fail}/>;
}
'''
(root/'src/app/match-2d-pitch.tsx').write_text(wrapper)

css=root/'src/app/match-2d-pitch.module.css'
text=css.read_text()
text+='''\n.phaserShell{position:relative;margin:10px 0 16px;border:1px solid rgba(255,255,255,.17);border-radius:14px;overflow:hidden;background:#174a31;min-height:390px;box-shadow:0 16px 30px rgba(0,0,0,.22)}.phaserHost{width:100%;height:min(62vh,620px);min-height:390px}.phaserHost canvas{display:block;width:100%!important;height:100%!important}.phaserLoading{position:absolute;inset:0;display:grid;place-items:center;background:#174a31;font-size:11px;font-weight:900;letter-spacing:.12em}.phaserCommentary{position:absolute;left:50%;bottom:13px;transform:translateX(-50%);z-index:4;display:flex;gap:8px;align-items:center;max-width:82%;padding:7px 11px;border-radius:9px;background:rgba(4,13,9,.78);backdrop-filter:blur(5px);font-size:10px;line-height:1.25;border:1px solid rgba(255,255,255,.12);color:#fff}.phaserCommentary b{font-size:11px}@media(max-width:720px){.phaserShell,.phaserHost{min-height:460px}.phaserHost{height:58vh}.phaserCommentary{max-width:91%;bottom:9px}}\n'''
css.write_text(text)

# tests cover that visual scene API still remains deterministic
p=root/'src/game-engine/match-2d-scene.test.ts'
if p.exists():
    t=p.read_text()
    if 'Phaser renderer contract' not in t:
