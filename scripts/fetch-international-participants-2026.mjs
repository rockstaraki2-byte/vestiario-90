...
for(const spec of SPECS){try{let found=null;for(const code of spec.codes){try{found=await participants(code);if(found?.ids?.length)break}catch{}}if(!found?.ids?.length)throw new Error(`${spec.id}: no valid participant source`);const clubIds=found.ids;const signature=[...clubIds].sort((a,b)=>a.localeCompare(b,undefined,{numeric:true})).join(",");const duplicate=signatures.get(signature);if(duplicate)throw new Error(`${spec.id}: participant set duplicates ${duplicate}`);signatures.set(signature,spec.id);if(clubIds.length!==spec.expected)throw new Error(`${spec.id}: unexpected participant count ${clubIds.length}/${spec.expected}`);if(new Set(clubIds).size!==clubIds.length)throw new Error(`${spec.id}: duplicate participant ids`);
