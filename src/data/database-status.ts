...
for(const comp of EUROPE_LOWER_2026_COMPETITIONS){const real=realLower.get(comp.id)??[],complete=real.length>=comp.clubs.length&&comp.clubs.length>0,status:DatabaseStatusLevel=complete?"updated":real.length?"partial":"pending";rows.push(row(comp.id,comp.name,comp.country,"Liga profissional",comp.clubs,"2026-09-09",status,real.length?`${real.length}/${comp.clubs.length} clubes com overlay real validado`:`Participantes cadastrados; elenco real completo ainda pendente`))}
