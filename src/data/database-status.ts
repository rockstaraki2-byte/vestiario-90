...
for(const comp of EUROPE_LOWER_2026_COMPETITIONS){const real=realLower.get(comp.id)??[],err=lowerErrors.get(comp.id),complete=real.length>=comp.clubs.length&&comp.clubs.length>0,status:DatabaseStatusLevel=err?"error":complete?"updated":real.length?"partial":"pending",clubs=real.length?real:comp.clubs;rows.push(row(comp.id,comp.name,comp.country,"Liga profissional",clubs,"2026-09-10",status,err?`Falha na sincronização: ${err}`:complete?`${real.length}/${comp.clubs.length} clubes com elenco real validado`:`${real.length}/${comp.clubs.length} clubes com elenco real; carga parcial`))}

const addedErrors=new Map<string,string>((ADDED_2026_SYNC_ERRORS as readonly SyncError[]).map(x=>[errorKey(x),String(x.error)]));
for(const comp of ADDED_2026_COMPETITIONS){const err=addedErrors.get(comp.id),status:DatabaseStatusLevel=err?"error":comp.clubs.length?"updated":"pending";rows.push(row(comp.id,comp.name,comp.country,"Liga profissional",comp.clubs,"2026-09-10",status,err?`Falha na sincronização: ${err}`:undefined))}
for(const [id,error] of addedErrors){if(rows.some(r=>r.id===id))continue;rows.push({id,name:id,country:"Internacional",category:"Liga profissional",status:"error",clubs:0,players:0,photoCoverage:0,crestCoverage:0,snapshot:"2026-09-10",note:`Falha na sincronização: ${error}`})}

for(const comp of YOUTH_WORLD_2026_COMPETITIONS)rows.push(row(comp.id,comp.name,comp.country,"Base",comp.clubs,"2026-09-10",comp.clubs.length?"updated":"pending",`${comp.level} • elenco real importado por competição`));

const stateLoaded=new Set<string>();
for(const comp of BRAZIL_STATE_2026_COMPETITIONS){stateLoaded.add(comp.id);const partial=Boolean(comp.coverage?.partialClubs?.length||comp.coverage?.clubErrors?.length),status:DatabaseStatusLevel=comp.clubs.length?(partial?"partial":"updated"):"pending",note=partial?`${comp.tier} • ${comp.clubs.length} clubes; ${comp.coverage?.partialClubs.length??0} elencos com cobertura baixa`:`${comp.tier} • clubes e elencos reais; identidade compartilhada com o mundo nacional`;rows.push(row(comp.id,comp.name,`Brasil/${comp.state}`,"Estadual",comp.clubs,BRAZIL_STATE_2026_META.snapshot,status,note))}
for(const item of BRAZIL_STATE_2026_SYNC_ERRORS as readonly SyncError[]){const id=errorKey(item);if(!id||stateLoaded.has(id))continue;rows.push({id,name:item.name??id,country:`Brasil/${item.state??"—"}`,category:"Estadual",status:"error",clubs:0,players:0,photoCoverage:0,crestCoverage:0,snapshot:BRAZIL_STATE_2026_META.snapshot,note:`${item.tier??"Estadual"} • falha na sincronização: ${item.error}`})}
...
for(const comp of INTERNATIONAL_2026_PARTICIPANTS){const clubs=intlRosters.get(comp.id)??[],err=intlErrors.get(comp.id),participantCount=comp.clubIds.length,complete=participantCount>0&&clubs.length>=participantCount&&clubs.every(c=>c.players.length>=15),status:DatabaseStatusLevel=err?"error":complete?"updated":clubs.length?"partial":"pending";rows.push(row(comp.id,comp.name,intlCountry(comp.id),"Internacional",clubs,"2026-09-10",status,err?`Falha na sincronização: ${err}`:complete?`${participantCount}/${participantCount} participantes com elenco real e identidade de jogadores`:`${clubs.length}/${participantCount} participantes com elenco real; sincronização parcial`))}
for(const [id,error] of intlErrors){if(rows.some(r=>r.id===id))continue;rows.push({id,name:id,country:intlCountry(id),category:"Internacional",status:"error",clubs:0,players:0,photoCoverage:0,crestCoverage:0,snapshot:"2026-09-10",note:`Falha na sincronização: ${error}`})}

const existing=new Set(rows.map(r=>r.id));
for(const comp of REAL_COMPETITION_CALENDAR){if(existing.has(comp.id))continue;const category:DatabaseStatusCategory=comp.scope==="state"?"Estadual":comp.scope==="domestic_cup"?"Copa nacional":comp.scope==="international_youth"||comp.scope==="national_youth"?"Internacional":"Base",status:DatabaseStatusLevel=comp.status==="confirmed"?"partial":"pending";rows.push({id:comp.id,name:comp.name,country:comp.country,category,status,clubs:0,players:0,photoCoverage:0,crestCoverage:0,snapshot:"2026-09-10",note:`Calendário ${comp.status==="confirmed"?"real confirmado; elencos/participantes em sincronização":comp.status==="partial"?"parcialmente confirmado":"aguardando datas oficiais"} • fonte: ${comp.source}${comp.note?` • ${comp.note}`:""}`})}
