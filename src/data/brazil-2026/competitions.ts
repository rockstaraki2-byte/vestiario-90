...
export type CompetitionDefinition = {
  id: GameCompetitionId;
  name: string;
  shortName: string;
  kind: CompetitionKind;
  country: string;
  season: number;
  selectableAsCareer: boolean;
  startDate: string;
  roundCadenceDays: number;
  doubleRoundRobin: boolean;
  benchSize: number;
  maxSubstitutions: number;
  clubs: CompetitionClubRoster[];
};
...
function disambiguateBrazilClub(id: "BRA2" | "BRA3", club: CompetitionClubRoster): CompetitionClubRoster {
  if (club.name !== "Botafogo FC") return club;
  return id === "BRA2"
    ? { ...club, name: "Botafogo-SP", shortName: "BOTAFOGO-SP" }
...
const get = (id: "BRA2" | "BRA3" | "CB20" | "SPjr") => {
  ...
  if (id === "BRA2" || id === "BRA3") {
    return applyBrazilRosterOverrides(id, value.clubs).map(club => disambiguateBrazilClub(id, club));
  }
  return value.clubs;
};
...
const EUROPE_COMPETITIONS: CompetitionDefinition[] = [...EUROPE_2026_COMPETITIONS, ...EUROPE_LOWER_2026_COMPETITIONS].map(item => ({
  id: item.id,
  name: item.name,
  ...
  season: item.season,
  selectableAsCareer: true,
  startDate: item.startDate,
  roundCadenceDays: item.roundCadenceDays,
  doubleRoundRobin: item.doubleRoundRobin,
  benchSize: item.benchSize,
  maxSubstitutions: item.maxSubstitutions,
  clubs: item.clubs,
}));

const ADDED_COMPETITIONS: CompetitionDefinition[] = ADDED_2026_COMPETITIONS.map(item => ({
  id: item.id,
  name: item.name,
  shortName: item.shortName,
  kind: "professional",
