...
function currentSquad(season:SeasonState,teamName:string):{rows:SquadRow[];label:string;asOf?:string}{
 ...
 if(official){
  const rows=official.players.map((item:RealNationalCallupPlayer,index)=>{const found=byName.get(norm(item.name));return{id:`official-${teamName}-${index}`,name:item.name,club:found?.club??item.club,position:item.position,overall:found?.player.overall,condition:found?.player.condition,form:found?.player.form,injuryDays:found?.player.injuryDays,player:found?.player,source:"oficial" as const}});
