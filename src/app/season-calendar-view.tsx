...
export default function SeasonCalendarView({season}:{season:SeasonState}){
 ...
 const items=useMemo(()=>{
  ...
  return{league,userCups,cupMilestones,nations,managedNation};
 },[season,club.id,leagueName]);
 const visible=useMemo(()=>{const source=filter==="Meu clube"?[...items.league,...items.userCups]:filter==="Copas"?[...items.userCups,...items.cupMilestones]:filter==="Seleções"?[...items.managedNation,...items.nations]:[...items.league,...items.userCups,...items.cupMilestones,...items.managedNation,...items.nations];return source.sort((a,b)=>a.date.localeCompare(b.date)||a.competition.localeCompare(b.competition));},[filter,items]);
