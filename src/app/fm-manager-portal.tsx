...
import { Activity, CalendarDays, ChevronRight, Clock3, Dumbbell, Shield, Sparkles, Target, UsersRound } from "lucide-react";
...
function TwoWeeks({season,onNavigate}:{season:SeasonState;onNavigate:(screen:string)=>void}){const fixtures=clubCommitments(season),days=Array.from({length:14},(_,index)=>addDays(season.currentDate,index));return <section className={styles.fortnight}><header><div><span>AGENDA DE PREPARAÇÃO</span><b>O que acontece nos próximos 14 dias</b></div><button onClick={()=>onNavigate("Calendário")}>Abrir calendário <ChevronRight/></button></header><div>{days.map((date,index)=>{const fixture=fixtures.find(item=>!item.played&&item.date===date),label=fixture?"Jogo • "+shortTeam(fixture.opponentName):index%7===0?"Recuperação":season.trainingPlan;return <button key={date} className={date===season.currentDate?styles.todayDay:fixture?styles.matchDay:""} onClick={()=>onNavigate(fixture?"Táticas":"Calendário")} title={fixture?fixture.competition+" • "+fixture.stage+" • "+fixture.opponentName:undefined}><small>{weekday(date)}</small><b>{date.slice(8)}</b><span>{label}</span></button>})}</div></section>}
