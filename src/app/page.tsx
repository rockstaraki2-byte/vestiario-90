...
export default function Dashboard(){
  ...
  return <div className={styles.shell}>
    ...
    <main className={styles.main}>
      <header className={styles.topbar}><div className={styles.mobileBrand}>V90</div><div className={styles.season}><span>{SOCCERWIKI_COMPETITION.name} • Temporada {season.year}</span><b>{season.completed?"TEMPORADA ENCERRADA":`RODADA ${season.currentRound} DE 38`} • {world.day} SET</b></div><div className={styles.topActions}><button aria-label="Mensagens"><MessageSquareText size={19}/><i/></button><button aria-label="Notificações"><Bell size={19}/><i/></button><button className={styles.advance} onClick={handleAdvance}>AVANÇAR <Play size={15} fill="currentColor"/></button></div></header>
...
function SquadView({club,lineupIds,onToggle}:{club:LeagueClub;lineupIds:string[];onToggle:(id:string)=>void}){return <section className={`${styles.card} ${styles.dataCard}`}><div className={styles.cardHead}><div>30 JOGADORES • {lineupIds.length}/11 TITULARES</div><button>{SOCCERWIKI_META.source} • snapshot {SOCCERWIKI_META.snapshot}</button></div><div className={styles.tableScroll}><table className={styles.dataTable}><thead><tr><th>XI</th><th>Jogador</th><th>Pos.</th><th>Idade</th><th>OVR</th><th>Condição</th><th>Fadiga</th><th>Moral</th><th>Forma</th><th>G</th><th>CA</th><th>Status</th></tr></thead><tbody>{club.players.map(p=>{const selected=lineupIds.includes(p.id),blocked=p.injuryDays>0||p.suspensionMatches>0;return <tr key={p.id}><td><button className={`${seasonStyles.xiButton} ${selected?seasonStyles.xiSelected:""}`} disabled={blocked} onClick={()=>onToggle(p.id)}>{selected?"✓":"+"}</button></td><td><b>{p.name}</b>{p.sourceId&&<small className={seasonStyles.sourceId}> SW#{p.sourceId}</small>}</td><td><span className={styles.position}>{p.position}</span></td><td>{p.age}</td><td><strong>{p.overall}</strong></td><td><Progress value={p.condition}/></td><td>{p.fatigue}%</td><td><Progress value={p.morale}/></td><td>{p.form.toFixed(1)}</td><td>{p.goals}</td><td>{p.yellowCards}</td><td><em className={blocked?seasonStyles.dangerStatus:""}>{p.status}</em></td></tr>})}</tbody></table></div></section>}
