...
import { advanceCalendarDay, createSeason, getCurrentUserFixture, getSelectedClub, getTodayUserFixture, getUserFixtures, hydrateSeasonState, playCurrentRound, resolveSeasonWorldChoice, startNextSeason, toggleLineupPlayer, type SeasonState } from "@/game-engine/season";

const NAV=[["Visão geral",Home],["Caixa de entrada",Inbox],["Elenco",Users],["Vestiário",MessageSquareText],["Clube",Building2],["Táticas",LayoutGrid],["Calendário",CalendarDays],["Classificação",Trophy],["Mercado",BarChart3],["Carreira",BriefcaseBusiness],["Notícias",Newspaper]] as const;

export default function Dashboard(){
  const [screen,setScreen]=useState<"menu"|"game">("menu");
  const [saveId,setSaveId]=useState<string|null>(null);
  const [season,setSeason]=useState<SeasonState>(()=>createSeason("vestiario-90",2026));
  ...
  function handleCareerAdvanceRound(){if(season.completed){handleNextSeason();return}const next=playCurrentRound(season);persist(next);flash(`Rodada ${Math.min(totalRounds,next.currentRound-1)} simulada • mercado de treinadores atualizado`)}
  ...
  return <div className={styles.shell}>
    <aside className={styles.sidebar}>
      ...
      <div className={styles.clubCard}>{employed?<ClubLogo club={club} size={38}/>:<BriefcaseBusiness size={28}/>}<div><b>{employed?club.name:"Sem clube"}</b><small>{employed?`${competition.name} • ${season.year}`:"Disponível no mercado"}</small></div><ChevronRight size={17}/></div>
      <button className={styles.settings} onClick={()=>setScreen("menu")}><Settings size={18}/> Menu principal</button>
    ...
    <main className={styles.main}>
      <header className={styles.topbar}><div className={styles.mobileBrand}>V90</div><div className={styles.season}><span>{employed?competition.name:"Mercado de Treinadores"} • Temporada {season.year}</span><b>{season.completed?"TEMPORADA ENCERRADA":`RODADA ${season.currentRound} DE ${totalRounds}`} • {formatDate(season.currentDate)}</b></div><div className={styles.topActions}><button aria-label="Mensagens" onClick={()=>setActive("Caixa de entrada")}><MessageSquareText size={19}/>{unreadEvents>0&&<i/>}</button><button aria-label="Notificações"><Bell size={19}/><i/></button><button className={styles.advance} onClick={handleAdvance}>{employed?"AVANÇAR DIA":"AVANÇAR RODADA"} <Play size={15} fill="currentColor"/></button></div></header>
