...
export default function Dashboard(){
  ...
  return <div className={styles.shell}>
    ...
    <main className={styles.main}>
      <header className={styles.topbar}><div className={styles.mobileBrand}>V90</div><div className={styles.season}><span>{SOCCERWIKI_COMPETITION.name} • Temporada {season.year}</span><b>{season.completed?"TEMPORADA ENCERRADA":`RODADA ${season.currentRound} DE 38`} • {world.day} SET</b></div><div className={styles.topActions}><button aria-label="Mensagens" onClick={()=>setActive("Vestiário")}><MessageSquareText size={19}/><i/></button><button aria-label="Notificações"><Bell size={19}/><i/></button><button className={styles.advance} onClick={handleAdvance}>AVANÇAR <Play size={15} fill="currentColor"/></button></div></header>
