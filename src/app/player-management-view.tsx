...
export default function PlayerManagementView({season,onToggleLineup,onResult}:{season:SeasonState;onToggleLineup:(id:string)=>void;onResult:(result:ClubActionResult)=>void}){
 ...
 return <div className={styles.layout}>
  <section className={styles.listPanel}>
   <header><div><span>ELENCO PROFISSIONAL</span><h2>{club.players.length} jogadores</h2></div><small>Toque em um atleta para abrir o dossiê</small></header>
   <div className={styles.tableWrap}><table><thead><tr><th>XI</th><th>Jogador</th><th>Pos.</th><th>Idade</th><th>OVR</th><th>Papel</th><th>Satisf.</th><th>Status</th></tr></thead><tbody>{sorted.map(item=>{const selected=item.id===player.id,blocked=item.injuryDays>0||item.suspensionMatches>0;return <tr key={item.id} className={selected?styles.selected:""} onClick={()=>setSelectedId(item.id)}><td data-label="XI"><button className={starters.has(item.id)?styles.xiOn:styles.xi} disabled={blocked} onClick={event=>{event.stopPropagation();onToggleLineup(item.id)}}>{starters.has(item.id)?"✓":"+"}</button></td><td data-label="Jogador"><div style={{display:"flex",alignItems:"center",gap:9,minWidth:0}}><PlayerPhoto player={item} size={36}/><span style={{minWidth:0}}><b>{item.name}</b><small>{item.personality}</small></span></div></td><td data-label="Posição">{shortPosition(item.position)}</td><td data-label="Idade">{item.age}</td><td data-label="OVR"><strong>{item.overall}</strong></td><td data-label="Papel">{item.squadRole}</td><td data-label="Satisfação"><Meter value={item.happiness}/></td><td data-label="Status"><em className={blocked?styles.danger:""}>{item.status}</em></td></tr>})}</tbody></table></div>
  </section>
  <aside className={styles.dossier}>
   <div className={styles.identity}><PlayerPhoto player={player} size={54} className={styles.avatar} eager/><div><span>DOSSIÊ DO JOGADOR</span><h2>{player.name}</h2><p>{player.position} • {player.age} anos • {player.squadRole}</p></div></div>
