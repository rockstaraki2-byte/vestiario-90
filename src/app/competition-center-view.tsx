...
function LeaguePanel({season}:{season:SeasonState}){
 const club=season.league.clubs.find(item=>item.id===season.selectedClubId)??season.league.clubs[0],standings=sortedStandings(season.league),competition=professionalCompetitionById(season.competitionId),fixtures=[...season.league.fixtures].filter(item=>item.played||Boolean(item.date&&item.date>=season.currentDate)).sort((a,b)=>(a.date??"9999-12-31").localeCompare(b.date??"9999-12-31")||a.round-b.round),currentIndex=Math.max(0,fixtures.findIndex(item=>!item.played&&Boolean(item.date&&item.date>=season.currentDate))),sliceStart=Math.max(0,currentIndex-10),visible=fixtures.slice(sliceStart,sliceStart+20);
