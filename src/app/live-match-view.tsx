...
import { liveCompetitionLabel, playerCompetitionGoalsAtMoment } from "@/game-engine/match-presentation";
...
export default function LiveMatchView({session,home,away,defaultSpeed,assistantTeamTalks,showStaffAdvice,parallelMatches=[],news=[],social=[],onChange,onFinish}:{session:LiveMatchState;home:LeagueClub;away:LeagueClub;defaultSpeed:MatchSpeed;assistantTeamTalks:boolean;showStaffAdvice:boolean;parallelMatches?:MatchdayParallelMatch[];news?:MatchdayNews[];social?:MatchdaySocial[];onChange:(next:LiveMatchState)=>void;onFinish:(session:LiveMatchState)=>void}){
  ...
  const [goalMoment,setGoalMoment]=useState<MatchEvent|null>(null);
  ...
  const goalScorer=goalMoment?.playerId?byId.get(goalMoment.playerId):undefined,goalClub=goalMoment?.team==="away"?away:home,goalCompetition=liveCompetitionLabel(session.seed),goalCompetitionTotal=goalScorer?playerCompetitionGoalsAtMoment(goalScorer,session.events,session.seed):0;

  useEffect(()=>{if(!goalKey||goalKey===seenGoal.current||!latestGoal)return;seenGoal.current=goalKey;const show=window.setTimeout(()=>{setGoalFlash(true);setGoalMoment(latestGoal)},0),hide=window.setTimeout(()=>{setGoalFlash(false);setGoalMoment(null)},3400);return()=>{window.clearTimeout(show);window.clearTimeout(hide)}},[goalKey,latestGoal]);
  ...
  useEffect(()=>{
    if(!running||goalMoment||!isLiveMatchRunning(session)||required.length)return;
  ...
  },[running,goalMoment,speed,session,home,away,onChange,required.length]);
  ...
  return <div className={liveStyles.layout}>
    {goalMoment&&<div role="status" aria-live="assertive" style={{position:"fixed",inset:0,zIndex:140,display:"grid",placeItems:"center",background:"rgba(4,10,8,.68)",backdropFilter:"blur(4px)",padding:18}}><div style={{width:"min(390px,92vw)",border:"1px solid #3d7656",borderRadius:18,background:"linear-gradient(145deg,#10251a,#0b1511)",boxShadow:"0 24px 80px #000b",padding:"22px 18px",display:"grid",justifyItems:"center",gap:9,textAlign:"center"}}><span style={{fontSize:10,fontWeight:950,letterSpacing:".16em",color:"#56db91"}}>GOOOOL • {goalMoment.minute}′</span><div style={{position:"relative"}}>{goalScorer?<PlayerPhoto player={goalScorer} size={96} eager/>:<div style={{width:96,height:96,borderRadius:"50%",display:"grid",placeItems:"center",background:"#173d2a",fontSize:32}}>⚽</div>}<div style={{position:"absolute",right:-8,bottom:-5,background:"#0b1511",border:"1px solid #315843",borderRadius:"50%",padding:5}}><TeamBadge name={goalClub.name} src={goalClub.imageUrl} size={28}/></div></div><h2 style={{margin:0,fontSize:28,color:"#f0f7f3"}}>{goalScorer?.name??"Gol"}</h2><strong style={{fontSize:14,color:"#63df9d"}}>{goalCompetitionTotal}º gol em {goalCompetition}</strong><small style={{fontSize:11,color:"#91a59b"}}>{goalClub.shortName} • {home.shortName} {session.homeGoals} × {session.awayGoals} {away.shortName}</small><button onClick={()=>setGoalMoment(null)} style={{marginTop:4,border:"1px solid #35654b",borderRadius:9,background:"#173d2a",color:"#7aeba9",padding:"9px 15px",fontSize:9,fontWeight:900,cursor:"pointer"}}>CONTINUAR</button></div></div>}
