...
export default function LiveMatchView({session,home,away,onChange,onFinish}:{session:LiveMatchState;home:LeagueClub;away:LeagueClub;onChange:(next:LiveMatchState)=>void;onFinish:(session:LiveMatchState)=>void}){
 ...
 useEffect(()=>{if(session.phase==="halftime"||session.phase==="fulltime"||required.length)setRunning(false)},[session.phase,required.length]);
 useEffect(()=>{
  if(!running||!isLiveMatchRunning(session)||required.length)return;
  const timer=window.setTimeout(()=>onChange(advanceLiveMatchMinute(session,home,away)),SPEED_MS[speed]);
 ...
 return <div className={liveStyles.layout}>
  <section className={liveStyles.scoreboard}>
   ...
   <div className={liveStyles.matchStats}><span>POSSE <b>{session.possessionHome}%</b>–<b>{100-session.possessionHome}%</b></span><span>FINALIZAÇÕES <b>{session.shotsHome}</b>–<b>{session.shotsAway}</b></span><span>TROCAS <b>{substitutions.length}/5</b></span></div>
  ...
  <section className={liveStyles.matchGrid}>
   <div className={liveStyles.pitchPanel}>
    ...
    <div className={liveStyles.pitch}>
     ...
     {homeAssignments.map(({slot,player})=><div key={`h-${player.id}`} className={`${liveStyles.pitchPlayer} ${liveStyles.homePlayer} ${session.userSide==="home"?liveStyles.userPlayer:""}`} style={{left:`${slot.x}%`,top:`${slot.y}%`}}><i>{player.position}</i><b>{player.name.split(" ")[0]}</b></div>)}
     {awayAssignments.map(({slot,player})=><div key={`a-${player.id}`} className={`${liveStyles.pitchPlayer} ${liveStyles.awayPlayer} ${session.userSide==="away"?liveStyles.userPlayer:""}`} style={{left:`${100-slot.x}%`,top:`${100-slot.y}%`}}><i>{player.position}</i><b>{player.name.split(" ")[0]}</b></div>)}
    </div>
   </div>
   ...
   <div className={liveStyles.controlColumn}>
    ...
    <div className={liveStyles.panel}>
     ...
     <div className={liveStyles.subRow}><label>SAI<select value={outId} onChange={e=>setOutId(e.target.value)}><option value="">Selecione</option>{userLineupIds.map(id=>{const p=playerById.get(id);return <option key={id} value={id}>{required.includes(id)?"🩺 ":""}{p?.position} • {p?.name}</option>})}</select></label><label>ENTRA<select value={inId} onChange={e=>setInId(e.target.value)}><option value="">Selecione</option>{userBenchIds.map(id=>{const p=playerById.get(id);return <option key={id} value={id}>{p?.position} • {p?.name} • OVR {p?.overall}</option>})}</select></label><button disabled={!outId||!inId||substitutions.length>=5||session.phase==="pre_match"||session.phase==="fulltime"} onClick={substitute}>TROCAR</button></div>
     {substitutions.length>0&&<div className={liveStyles.subHistory}>{substitutions.slice().reverse().map((sub,i)=><span key={`${sub.outPlayerId}-${i}`}><small>{sub.minute}'</small> {playerById.get(sub.outPlayerId)?.name} → <b>{playerById.get(sub.inPlayerId)?.name}</b></span>)}</div>}
    </div>
   </div>
  </section>

  <section className={liveStyles.commentary}><header><b>NARRAÇÃO EVENTO A EVENTO</b><small>{session.events.length} acontecimentos • relógio {session.currentMinute}'</small></header><div>{session.events.slice().reverse().map((event,index)=><article key={`${event.minute}-${event.type}-${index}`} className={event.type==="goal"?liveStyles.goal:event.type==="injury"?liveStyles.injury:""}><time>{event.minute===0?"00'":`${event.minute}'`}</time><span>{event.type==="goal"?"⚽":event.type==="card"?"🟨":event.type==="injury"?"🩺":event.type==="halftime"?"⏸":"•"}</span><p>{event.text}</p></article>)}</div></section>

  <section className={liveStyles.squads}><div><b>{userClub.name} EM CAMPO</b>{userLineupIds.map(id=>{const p=playerById.get(id);return <span key={id}><i>{p?.position}</i>{p?.name}<small>{p?.condition}%</small></span>})}</div><div><b>BANCO</b>{userBenchIds.slice(0,9).map(id=>{const p=playerById.get(id);return <span key={id}><i>{p?.position}</i>{p?.name}<small>OVR {p?.overall}</small></span>})}</div></section>
 </div>;
}
