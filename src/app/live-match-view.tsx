...
import { makeSubstitution, playFinalMinutes, playFirstHalf, playSecondHalf, requiredUserSubstitutions, setTeamTalk, updateLiveTactic, type LiveMatchState, type TeamTalk } from "@/game-engine/live-match";
...
export default function LiveMatchView({session,home,away,onChange,onFinish}:{session:LiveMatchState;home:LeagueClub;away:LeagueClub;onChange:(next:LiveMatchState)=>void;onFinish:(session:LiveMatchState)=>void}){
  ...
  const decisionPhase=session.phase==="halftime"||session.phase==="second_half_window";
  ...
  return <div className={liveStyles.layout}>
    <section className={liveStyles.scoreboard}>
      <span>{session.phase==="pre_match"?"PRÉ-JOGO":session.phase==="halftime"?"INTERVALO":session.phase==="second_half_window"?"70 MINUTOS":"ENCERRADO"}</span>
    ...
    {decisionPhase&&<section className={liveStyles.decisions}>
      {session.phase==="halftime"&&<div className={liveStyles.panel}>
        <header><Users size={16}/><div><b>CONVERSA NO INTERVALO</b><small>A fala gera um impulso contextual que se dissipa ao longo do segundo tempo.</small></div></header>
      ...
      <div className={liveStyles.panel}>
        <header><Repeat2 size={16}/><div><b>SUBSTITUIÇÕES • {substitutions.length}/5</b><small>{session.phase==="halftime"?"Mexa no time para a volta do intervalo.":"Última janela de decisão antes dos 20 minutos finais."}</small></div></header>
      ...
      <div className={liveStyles.panel}>
        <header><ShieldAlert size={16}/><div><b>AJUSTE TÁTICO</b><small>{session.phase==="halftime"?"Vale a partir do reinício.":"Vale para os 20 minutos decisivos."}</small></div></header>
