...
export function deleteSaveSlot(id:string,explicit?:StorageLike){const s=storage(explicit);if(!s)return;try{const index=listSaveSlots(s).filter(item=>item.id!==id);writeIndex(s,index);s.removeItem(`${SAVE_PREFIX}${id}`);if(s.getItem(ACTIVE_SAVE_KEY)===id)s.removeItem(ACTIVE_SAVE_KEY);}catch{return;}}
