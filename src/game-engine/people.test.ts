...
describe("pessoas e vestiário",()=>{
 ...
 it("promessa de minutos é criada e pode ser cumprida",()=>{let season=createSeason("promise");const club=season.league.clubs[0],player=club.players.find(p=>season.lineupIds.includes(p.id))!;season=talkToPlayer(season,player.id,"Prometer minutos").state;for(let i=0;i<3;i++)season=playCurrentRound(season);const updated=season.league.clubs[0].players.find(p=>p.id===player.id)!;expect(updated.promises[0].status).toBe("Cumprida");expect(updated.appearances).toBeGreaterThanOrEqual(3)},15_000);
