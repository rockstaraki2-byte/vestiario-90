...
function applyPlayerEffects(club:LeagueClub,result:MatchResult,side:"home"|"away",participatingIds:string[],seed:string,competitionId:ProfessionalCompetitionId){
  ...
  for(const player of club.players){if(player.suspensionMatches>0&&!participantIds.has(player.id))player.suspensionMatches=Math.max(0,player.suspensionMatches-1);if(participantIds.has(player.id)){player.condition=Math.max(45,player.condition-rng.integer(6,14));player.fatigue=Math.min(100,player.fatigue+rng.integer(8,16));player.morale=Math.max(0,Math.min(100,player.morale+moraleDelta+rng.integer(-1,1)));}else{player.condition=Math.min(100,player.condition+1);player.fatigue=Math.max(0,player.fatigue-2);}}
...
export function playCurrentRound(state:SeasonState,userTactic:MatchTactic=DEFAULT_TACTIC,userMatchOverride?:MatchResult,userParticipantIds?:string[]):SeasonState{
  ...
  for(const fixture of roundFixtures){
    ...
    const result=isUserFixture&&userMatchOverride?userMatchOverride:simulateMatch(home,away,`${state.baseSeed}:${state.year}:r${state.currentRound}:${fixture.id}`,userHome?userTactic:DEFAULT_TACTIC,userAway?userTactic:DEFAULT_TACTIC,homeStarters,awayStarters);
  ...
  const selection=normalizeMatchdaySelection({...state,league} as SeasonState,selectedClub),livingWorldBase=state.livingWorld??createLivingWorld(selectedClub.name);let livingWorld=userResult?worldAfterMatch(livingWorldBase,selectedClub,state.currentRound,gf,ga):livingWorldBase;const careerResult=careerAfterRound(state.career??createManagerCareer(selectedClub,state.year),league,livingWorld,state.currentRound,state.year,state.baseSeed,userResult?{goalsFor:gf,goalsAgainst:ga}:undefined);livingWorld=careerResult.world;
  const nextState:SeasonState={...state,league,currentRound:nextRound,lineupIds:selection.lineupIds,benchIds:selection.benchIds,recentForm,completed,championClubId,lastUserMatch,livingWorld,career:careerResult.career};return processMarketRound(processClubManagementRound(nextState));
}
...
export function advanceSeasonDay(state:SeasonState,calendarDay=1):SeasonState{
  ...
  const selection=normalizeMatchdaySelection({...state,league} as SeasonState,club);let livingWorld=state.livingWorld??createLivingWorld(club.name);if(state.career?.status!=="Sem clube"){livingWorld=worldAfterDay(livingWorld,club,state.currentRound,state.baseSeed);livingWorld=applyNarrativeDay(livingWorld,{day:calendarDay,round:state.currentRound,selectedClubId:state.selectedClubId,seed:state.baseSeed,league});}return processMarketRound({...state,league,lineupIds:selection.lineupIds,benchIds:selection.benchIds,livingWorld});
}
