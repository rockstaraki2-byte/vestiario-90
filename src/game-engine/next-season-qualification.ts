...
const NAME_EQUIV:Record<string,string>={
 "man city":"manchester city","man utd":"manchester united","paris":"paris saint germain","atleti":"atletico de madrid",
 "vasco":"vasco da gama","atletico mg":"atletico mineiro","red bull bragantino sp":"red bull bragantino","sao paulo futebol clube":"sao paulo",
};
...
function key(value:string){const normalized=value.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/\b(fc|afc|cf|ec|sc|saf)\b/g," ").replace(/[^a-z0-9]+/g," ").replace(/\s+/g," ").trim();return NAME_EQUIV[normalized]??normalized;}
...
export function realContinentalFallbackPool(id:InternationalCupEngineId):QualificationEntrant[]{
 ...
 for(const competitionId of ids){
  ...
  for(const club of field.clubs){
   if(result.some(item=>equivalent(item.name,club.name)))continue;
...
export function completeRealQualificationField(base:QualificationEntrant[],id:InternationalCupEngineId,count:number,blocked=new Set<string>()){
 ...
 const normalizedBlocked=[...blocked].map(key);
 const isBlocked=(name:string)=>normalizedBlocked.some(value=>equivalent(value,name));
 const add=(entry:QualificationEntrant)=>{
  if(isBlocked(entry.name)||out.some(item=>equivalent(item.name,entry.name))||out.length>=count)return;
  out.push(entry);
 };
 base.forEach(add);
