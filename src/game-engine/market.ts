...
import { negotiationStaffBonus, playerTransferInterest, renewalPersonalityModifier } from "./football-depth";
...
export function renewPlayerContract(state:SeasonState,playerId:string,years=3):MarketActionResult{
  ...
  const personalityBonus=(player.personality==="Leal"?12:player.personality==="Ambicioso"?-5:0)+renewalPersonalityModifier(player,state.baseSeed)+negotiationStaffBonus(state.depth);
...
export function concludeAcceptedOffer(state:SeasonState,offerId:string):MarketActionResult{
  ...
  const mood=(player.happiness<55?18:0)+(player.wantsToLeave?20:0),interest=playerTransferInterest(player,buyer,seller,offer.salaryBrlMonthly,state.baseSeed);
  const chance=Math.max(15,Math.min(96,52+prestige+raise+mood+(player.personality==="Ambicioso"?5:0)+(interest.score-50)*.55+negotiationStaffBonus(state.depth)));
  if(rng.integer(1,100)>chance){offer.status="Recusada";offer.message=`${player.name} e ${player.contract.agentName} recusaram os termos pessoais. ${interest.summary}`;return{state:{...state,league,market},message:offer.message};}
