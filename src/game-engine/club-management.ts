...
export function requestBoardTransferContact(state:ManagedSeason,targetPlayerId:string,type:TransferOfferType="Compra"):ClubActionResult{
  ...
  const fee=type==="Empréstimo"?Math.max(100_000,Math.round(recommendedOffer(player)*.06/50_000)*50_000):recommendedOffer(player),salary=Math.round(player.contract.salaryBrlMonthly*(type==="Compra"?1.14:1.05)/5_000)*5_000,wageHeadroom=Math.max(0,user.wageBudgetBrlMonthly-clubWageSpend(user)),same=user.players.filter(p=>p.position===player.position),sector=same.length?same.reduce((s,p)=>s+p.overall,0)/same.length:65,technicalGap=player.overall-sector,budgetRatio=fee/Math.max(1,user.transferBudgetEur),wageRatio=salary/Math.max(1,wageHeadroom),viability:BoardRequestRecord["viability"]=budgetRatio>1.25||wageRatio>1.35?"Inviável":budgetRatio>.82||wageRatio>.85||technicalGap<-2?"Atenção":"Viável";
...
export function requestBoardContractRenewal(state:ManagedSeason,playerId:string):ClubActionResult{
  const club=selectedClub(state),player=club.players.find(p=>p.id===playerId);if(!player)return{state,message:"Jogador não encontrado no elenco."};
