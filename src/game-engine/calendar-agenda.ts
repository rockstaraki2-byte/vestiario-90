...
const sortMatches=(items:CalendarAgendaMatch[])=>[...items].sort((a,b)=>a.date.localeCompare(b.date)||a.competition.localeCompare(b.competition)||a.id.localeCompare(b.id));
...
function cupAggregate(tournament:SeasonState["worldCompetitions"]["tournaments"][number],match:SeasonState["worldCompetitions"]["tournaments"][number]["matches"][number]){
 ...
 const completedLegs=played.length>=2;
 return `${completedLegs?"Agregado":"Agregado parcial"} ${homeTotal}–${awayTotal}`;
}
...
export function buildCalendarAgenda(season:SeasonState):CalendarAgenda{
 ...
 const mainClubs=new Map(season.league.clubs.map(club=>[club.id,club]));
 ...
 for(const tournament of season.worldCompetitions?.tournaments??[]){
  for(const match of tournament.matches){
   ...
   all.push({id:`cup-${tournament.definition.id}-${match.id}`,date:match.date,competition:tournament.definition.shortName,stage:match.stage,kind:"Copa",homeName:match.home.name,awayName:match.away.name,homeLogo:homeClub?.imageUrl,awayLogo:awayClub?.imageUrl,played,score:played?`${match.homeGoals} × ${match.awayGoals}`:undefined,status:played?"Final":match.date===season.currentDate?"Hoje":"Agendado",isUserClub:isUser,aggregate:cupAggregate(tournament,match),decidedByPenalties:match.decidedByPenalties,originalDate:match.originalDate,rescheduledReason:match.rescheduledReason});
  }
 }
 ...
 const managed=(season.nationalCareer?.fixtures??[]).map(fixture=>({id:`managed-national-${fixture.id}`,date:fixture.date,competition:fixture.competition,stage:fixture.stage,kind:"Seleção" as const,homeName:fixture.homeName,awayName:fixture.awayName,played:Boolean(fixture.played),score:fixture.played?`${fixture.homeGoals??0} × ${fixture.awayGoals??0}`:undefined,status:fixture.played?"Final":fixture.date===season.currentDate?"Hoje":"Agendado",isManagedNation:true}));
 const deduped=new Map<string,CalendarAgendaMatch>();for(const item of [...all,...managed])if(!deduped.has(item.id))deduped.set(item.id,item);
 const allGames=sortMatches([...deduped.values()]),myGames=sortMatches(allGames.filter(item=>item.isUserClub||item.isManagedNation));
...
export function compactAgendaMatches(matches:CalendarAgendaMatch[],currentDate:string,visibleFutureDates:number,recentDateCount=0){
 const sorted=sortMatches(matches),future=sorted.filter(item=>item.date>=currentDate),futureKeys=[...new Set(future.map(item=>item.date))].slice(0,Math.max(1,visibleFutureDates));
 let recentKeys:string[]=[];
 if(recentDateCount>0){const pastKeys=[...new Set(sorted.filter(item=>item.date<currentDate).map(item=>item.date))];recentKeys=pastKeys.slice(-recentDateCount);}
 const allowed=new Set([...recentKeys,...futureKeys]);return sorted.filter(item=>allowed.has(item.date));
}

export function agendaRemainingFutureDates(matches:CalendarAgendaMatch[],currentDate:string,visibleFutureDates:number){const total=[...new Set(matches.filter(item=>item.date>=currentDate).map(item=>item.date))].length;return Math.max(0,total-Math.max(1,visibleFutureDates));}
