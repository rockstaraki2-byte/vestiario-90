...
export type YouthIntakeRecord={year:number;clubId:string;clubName:string;playerIds:string[];bestPotential:number};
export type PlayerLifecycleState={lastProcessedYear?:number;retirements:PlayerRetirementRecord[];youthIntakes:YouthIntakeRecord[];squadTargets:Record<string,number>};
...
function homeNationality(club:LeagueClub){const counts=new Map<string,number>();for(const player of club.players){if(!player.nationality)continue;counts.set(player.nationality,(counts.get(player.nationality)??0)+1);}return[...counts.entries()].sort((a,b)=>b[1]-a[1])[0]?.[0]??"Brasil";}
...
function retirementChance(player:LeaguePlayer,year:number){
  ...
  if(player.contract.endYear<=year)chance+=.08;if(player.personality==="Profissional")chance-=.04;if(player.personality==="Competitivo")chance-=.02;return clamp(chance,.01,.98);
}
function shouldRetire(player:LeaguePlayer,seed:string,year:number){const keeper=player.position==="GOL",hardCap=keeper?43:41;if(player.age>=hardCap)return{retire:true,reason:"Limite de idade" as const};const rng=new SeededRng(`${seed}:retirement:${year}:${player.id}`);return{retire:rng.next()<retirementChance(player,year),reason:"Aposentadoria" as const};}
function academyPosition(club:LeagueClub,index:number){const counts=new Map<string,number>();for(const player of club.players)counts.set(player.position,(counts.get(player.position)??0)+1);const scarce=[...POSITIONS].sort((a,b)=>(counts.get(a)??0)-(counts.get(b)??0));return scarce[index%Math.min(4,scarce.length)]??POSITIONS[index%POSITIONS.length];}
function createAcademyPlayer(club:LeagueClub,index:number,year:number,seed:string):LeaguePlayer{
  const rng=new SeededRng(`${seed}:academy:${year}:${club.id}:${index}`),age=rng.integer(17,19),nationality=homeNationality(club),pool=namePool(nationality),position=academyPosition(club,index),reputationBase=Math.round(club.reputation*.52),overall=clamp(reputationBase+rng.integer(12,23),48,72),potential=clamp(overall+rng.integer(8,19)+(rng.integer(1,100)<=8?rng.integer(2,5):0),overall,95),name=`${rng.pick([...pool.first])} ${rng.pick([...pool.last])}`;
  const player:LeaguePlayer={id:`${club.id}-academy-${year}-${index}`,transfermarktId:`academy-${year}-${club.id}-${index}`,name,position,age,nationality,marketValueEur:null,overall,potential,seasonStartOverall:overall,developmentProgress:0,overallHistory:[{year,round:0,overall,reason:"início"}],morale:74,condition:98,fatigue:1,form:6.4,goals:0,assists:0,shots:0,yellowCards:0,redCards:0,wins:0,draws:0,losses:0,cleanSheets:0,ratingTotal:0,ratedMatches:0,averageRating:0,lastRating:0,injuryDays:0,suspensionMatches:0,status:"Promessa",personality:rng.pick(PERSONALITIES),squadRole:"Promessa",happiness:78,managerTrust:60,appearances:0,starts:0,minutes:0,promises:[],contract:{salaryBrlMonthly:Math.max(20_000,Math.round(overall*750/5_000)*5_000),startYear:year,endYear:year+4,agentName:"Representante da base",releaseClauseEur:null},transferListed:false,wantsToLeave:false,joinedClubYear:year,clubTrainedYears:0,associationTrained:false};
  Object.assign(player,{generated:true,academyOriginClubId:club.id,academyOriginClubName:club.name,generatedYear:year});return player;
}
...
export function applyPlayerLifecycleTransition(league:LeagueWorld,lifecycle:PlayerLifecycleState|undefined,seed:string,year:number):PlayerLifecycleTransition{
  const state=structuredClone(hydratePlayerLifecycleState(lifecycle));if(state.lastProcessedYear===year)return{state,retired:[],generated:[]};const retired:PlayerRetirementRecord[]=[],generated:LeaguePlayer[]=[];
  for(const club of league.clubs){
    const before=club.players.length,target=state.squadTargets[club.name]??Math.max(22,Math.min(32,before));state.squadTargets[club.name]=target;const survivors:LeaguePlayer[]=[];
    for(const player of club.players){const decision=shouldRetire(player,seed,year);if(!decision.retire){survivors.push(player);continue;}retired.push({year,playerId:player.id,name:player.name,clubId:club.id,clubName:club.name,position:player.position,age:player.age,overall:player.overall,peakOverall:peakOverall(player),appearances:player.appearances??0,goals:player.goals??0,assists:player.assists??0,reason:decision.reason});}
    club.players=survivors;const young=club.players.filter(player=>player.age<=20).length,academyNeed=Math.max(0,3-young),desired=Math.max(target,Math.min(32,club.players.length+academyNeed)),intake:LeaguePlayer[]=[];
