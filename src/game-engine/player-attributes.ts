...
export function playerAttributeProfile(player:LeaguePlayer):PlayerAttributeProfile{
 const personality=personalityProfile(player),agePhysical=player.age<=23?3:player.age>=33?-5:player.age>=30?-2:0,formBoost=Math.max(-3,Math.min(3,Math.round((player.form-6)/1.4))),attrs=definitions.map(([key,label,group])=>{const mental=group==="Mental"?(personality.professionalism-50)*.035+(personality.pressure-50)*.025:0,physical=group==="Físico"?agePhysical-(player.fatigue/100)*2:0,value=clamp(player.overall+positionBoost(player,key)+jitter(player,key)+mental+physical+formBoost+(player.trainingAttributeBoosts?.[key]??0),30,96);return{key,label,group,value};});
 const hidden:HiddenPlayerTraits={consistencia:clamp(45+personality.professionalism*.35+personality.pressure*.25+jitter(player,"cons",10)+(player.mentorMentalBoost??0),20,95),jogosGrandes:clamp(personality.pressure+jitter(player,"big",8),20,96),profissionalismo:clamp(personality.professionalism+(player.mentorMentalBoost??0),20,99),ambicao:personality.ambition,lealdade:personality.loyalty,adaptabilidade:personality.adaptability,propensaoLesao:clamp(24+player.age*.8+player.fatigue*.25+(100-player.condition)*.15+jitter(player,"inj",10),8,85)};
